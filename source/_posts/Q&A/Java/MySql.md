...
|异常类型|出现场景|描述对象|
|---|---|---|
|脏写|事务A、B同时写同一条数据，事务A提交，事务B回滚，此时事务A脏写|单行数据操作事务并发异常|
|脏读|事务A读取事务B未提交数据，然后事务B又选择了回滚，此时事务A出现脏读|单行数据操作事务并发异常|
|不可重复读|事务A对某行数据进行多次读取，此时事务B对该行数据进行了更新，事务A多次读取该行数据出现了不一致，即事务A出现不可重复读|单行数据操作事务并发异常|
|幻读|事务A正在对表进行多次查询，事务B此时在表中增删了多条数据，事务A多次查询结果不一致，此时事务A出现幻读|多行数据操作事务并发异常|
...
|可选配置值|简介|特点|
|---|---|---|
|READ-UNCOMMITTED|未提交读，最低隔离级别，事务未提交前，就可被其他事务读取，不允许两个未提交的事务同时修改一条数据|解决脏写，会出现脏读、幻读、不可重复读|
