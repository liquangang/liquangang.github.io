...
|面向过程、面向对象区别|面向过程是直接编写解决问题的所有流程细节，面向对象是将问题抽象成类的属性或行为，然后调用对象的属性或行为来解决问题|
...
|按流向分|输入流、输出流|
|按类型分|字节流、字符流|
|按功能分|缓冲流、打印流、序列化流、转换流|

## BIO、NIO、AIO 有什么区别？
||BIO|NIO|AIO|
|---|---|---|---|
|全称|Block IO同步阻塞式IO|New IO同步非阻塞式IO|Asynchronous IO，NIO升级版，异步非阻塞IO|
